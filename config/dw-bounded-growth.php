...
return [
    ...
    'cache_keys' => [
        ...
        'long_poll_wait_slots' => [
            ...
            'admission' => 'Empty workflow, activity, and query-task worker long-poll waits must acquire a slot before sleeping. Query-task polls use a separate pool so live workflow queries are not starved by idle workflow/activity waits. If all slots for a pool are occupied, guarded worker polls return Retry-After backpressure instead of creating an immediate empty-poll loop; remote runtimes receive HTTP 429 while PHP retains a protocol-level HTTP 200 compatibility response for earlier Workflow package workers. Pending tasks are claimed by the initial probe before any idle wait slot or backpressure response is required.',
