...
PY);
    ...
    public function test_python_baseline_retains_version_and_phase_when_worker_start_fails(): void
    {
        $result = $this->runSignalQueryRunnerPythonSnippet(<<<'PY'
versions = {
    "server": "2.0.0-beta.17",
    "cli": "2.0.0-beta.17",
    "sdk-python": "2.0.0-beta.17",
    "workflow": "2.0.0-beta.17",
    "sdk-php": "2.0.0-beta.17",
    "waterline": "2.0.0-beta.17",
}
sources = {
    "server": "published_docker_image",
    "cli": "published_cli_release",
...
PY);
        ...
        $this->assertSame('fail', $result['status']);
        $this->assertSame('2.0.0b17', $result['sdk_version']);
        $this->assertSame('2.0.0b17', $result['release_identity']);
    ...
    {
        $result = $this->runSignalQueryRunnerPythonSnippet(<<<'PY'
globals()["artifact_versions"] = {"sdk-python": "2.0.0-beta.17"}

print(json.dumps({
    "semver": python_sdk_version_matches_current("2.0.0-beta.17"),
    "pep440": python_sdk_version_matches_current("2.0.0b17"),
    "registration_pep440": python_sdk_version_matches_current(
        python_registration_release_version("durable-workflow-python/2.0.0b17"),
    ),
    "different_beta": python_sdk_version_matches_current("2.0.0b9"),
    "post_release": python_sdk_version_matches_current("2.0.0b17.post1"),
...
cases = {
    "missing": None,
    "tag_only": "docker.io/durableworkflow/waterline:2.0.0-beta.17",
...
PY);
        ...
        $this->assertSame('2.0.0-beta.17', $result['mapped_version']);
        $this->assertSame('oci', $result['identity']['kind']);
        $this->assertSame(
            'oci:docker.io/durableworkflow/waterline@2.0.0-beta.17',
...
def fake_run_command(command, **kwargs):
    ...
    if command[-2] == "{{json .Config.Labels}}":
        return subprocess.CompletedProcess(command, 0, json.dumps({
            ...
            "dev.durable-workflow.release.tag": "2.0.0-beta.17",
        }), "")
...
PY);
        ...
        $this->assertSame('2.0.0-beta.17', $result['source_revision_labels']['release_tag']);
        $this->assertSame(
            str_repeat('d', 40),
...
PY);
    ...
    {
        $result = $this->completeSignalQueryResult();
        $result['artifactVersions']['sdk-python'] = '2.0.0-beta.17';
        $python = &$result['scenario_results']['python_worker_cli_and_sdk_baseline']['observed_outputs'];
        $python['python_worker_sdk_version'] = '2.0.0b17';
        $python['routed_current_query_task']['worker_sdk_version'] = '2.0.0b17';
        $python['readiness_boundary'] = $this->pythonReadinessBoundaryEvidence('2.0.0b17');
        $python['controlled_restart'] = $this->pythonControlledRestartEvidence('2.0.0b17');
