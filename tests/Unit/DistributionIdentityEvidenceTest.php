...
final class DistributionIdentityEvidenceTest extends TestCase
{
    ...
    {
        $wheel = $this->root.'/durable_workflow-2.0.0b17-py3-none-any.whl';
        file_put_contents($wheel, 'beta four wheel bytes');
        $store = $this->root.'/python-prerelease.json';

        $semver = $this->runCommand('record-file', $store, 'sdk-python', '2.0.0-beta.17', $wheel);
        $pep440 = $this->runCommand('record-file', $store, 'sdk-python', '2.0.0b17', $wheel);
        ...
        $this->assertSame('pypi:durable-workflow@2.0.0b17', $identities['sdk-python']['locator']);
