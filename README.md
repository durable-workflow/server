...
```bash
export DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.4
...
- `DW_SERVER_TAG=2.0.0-beta.4` pulls `durableworkflow/server:2.0.0-beta.4` from Docker Hub.
- `DW_SERVER_IMAGE=ghcr.io/durable-workflow/server:2.0.0-beta.4` pulls the same release
...
export DW_SERVER_TAG=2.0.0-beta.4
export DW_AUTH_TOKEN=dev-token
...
```env
DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.4
SERVER_PORT=8080
APP_ENV=production
APP_DEBUG=false
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-beta.4`, matching
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-beta.4` tag
...
`durable-workflow/workflow:2.0.0-beta.4` package and verifies that the tag
resolves to commit `187746bb19615a8cbb25dfbe1e4e27dbbd933472` before the
...
git tag 2.0.0-beta.4 origin/v2
git push origin refs/tags/2.0.0-beta.4

# In the server repo, publish the Docker image tags.
git tag 2.0.0-beta.4 origin/main
git push origin refs/tags/2.0.0-beta.4
```
...
```bash
docker pull durableworkflow/server:2.0.0-beta.4
docker run --rm --entrypoint sh durableworkflow/server:2.0.0-beta.4 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'

docker pull ghcr.io/durable-workflow/server:2.0.0-beta.4
docker run --rm --entrypoint sh ghcr.io/durable-workflow/server:2.0.0-beta.4 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'
```
...
`durableworkflow/server:2.0.0-beta.4`. For production, patch every workload to the exact
