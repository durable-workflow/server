...
curl -X POST http://localhost:8080/api/worker/register \
  ...
  -H "X-Durable-Workflow-Protocol-Version: 1.11" \
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-alpha.223`, matching
...
The current server advertises worker protocol `1.11` by default. Worker-plane
...
equal to the server minor: a server advertising `1.11` accepts worker requests
for `1.0` through `1.11`, including older `1.10` and `1.9` workers. Requests
...
Within worker protocol version `1.11`, `worker_protocol.version`,
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-alpha.223` tag
