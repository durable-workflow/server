...
```bash
export DW_SERVER_IMAGE=durableworkflow/server:2.0.0-rc.10
...
- `DW_SERVER_TAG=2.0.0-rc.10` pulls `durableworkflow/server:2.0.0-rc.10` from Docker Hub.
- `DW_SERVER_IMAGE=ghcr.io/durable-workflow/server:2.0.0-rc.10` pulls the same release
...
export DW_SERVER_TAG=2.0.0-rc.10
export DW_AUTH_TOKEN=dev-token
...
```env
DW_SERVER_IMAGE=durableworkflow/server:2.0.0-rc.10
SERVER_PORT=8080
APP_ENV=production
APP_DEBUG=false
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-rc.7`, matching
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-rc.7` tag
...
`durable-workflow/workflow:2.0.0-rc.7` package and verifies that the tag
resolves to commit `44cb40197d5d2809f95b787ca073fadd33a6d577` before the
image can be published.

The current server image embeds the already-published Workflow `2.0.0-rc.7`
...
git tag 2.0.0-rc.10 origin/main
git push origin refs/tags/2.0.0-rc.10
```
...
```bash
docker pull durableworkflow/server:2.0.0-rc.10
docker run --rm --entrypoint sh durableworkflow/server:2.0.0-rc.10 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'

docker pull ghcr.io/durable-workflow/server:2.0.0-rc.10
docker run --rm --entrypoint sh ghcr.io/durable-workflow/server:2.0.0-rc.10 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'
```
...
`durableworkflow/server:2.0.0-rc.10`. For production, patch every workload to the exact
