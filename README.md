...
```bash
export DW_SERVER_IMAGE=durableworkflow/server:2.0.0-rc.5
...
- `DW_SERVER_TAG=2.0.0-rc.5` pulls `durableworkflow/server:2.0.0-rc.5` from Docker Hub.
- `DW_SERVER_IMAGE=ghcr.io/durable-workflow/server:2.0.0-rc.5` pulls the same release
...
export DW_SERVER_TAG=2.0.0-rc.5
export DW_AUTH_TOKEN=dev-token
...
```env
DW_SERVER_IMAGE=durableworkflow/server:2.0.0-rc.5
SERVER_PORT=8080
APP_ENV=production
APP_DEBUG=false
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-rc.5`, matching
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-rc.5` tag
...
`durable-workflow/workflow:2.0.0-rc.5` package and verifies that the tag
resolves to commit `d8356318311cf594ba47e685ec8e7654c7258a28` before the
...
git tag 2.0.0-rc.5 origin/v2
git push origin refs/tags/2.0.0-rc.5

# In the server repo, publish the Docker image tags.
git tag 2.0.0-rc.5 origin/main
git push origin refs/tags/2.0.0-rc.5
```
...
```bash
docker pull durableworkflow/server:2.0.0-rc.5
docker run --rm --entrypoint sh durableworkflow/server:2.0.0-rc.5 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'

docker pull ghcr.io/durable-workflow/server:2.0.0-rc.5
docker run --rm --entrypoint sh ghcr.io/durable-workflow/server:2.0.0-rc.5 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'
```
...
`durableworkflow/server:2.0.0-rc.5`. For production, patch every workload to the exact
