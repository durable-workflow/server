...
curl -X POST http://localhost:8080/api/worker/register \
  ...
  -H "X-Durable-Workflow-Protocol-Version: 1.10" \
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-alpha.218`, matching
...
The current server advertises worker protocol `1.10` by default. Worker-plane
...
equal to the server minor: a server advertising `1.10` accepts worker requests
for `1.0` through `1.10`, including older `1.9` workers. Requests with a missing,
...
Within worker protocol version `1.10`, `worker_protocol.version`,
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-alpha.218` tag
...
| `DW_QUERY_TASK_TIMEOUT` | `max(DW_WORKER_POLL_TIMEOUT + 15, DW_WORKFLOW_TASK_TIMEOUT + 5, 40)` | Seconds the control plane waits for a worker query response. The default covers a full worker long-poll cycle plus dispatch grace and the workflow-task replay barrier so first queries do not time out while a query-capable worker is rotating polls or finishing an active workflow task. |
