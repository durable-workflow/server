...
```bash
export DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.16
...
- `DW_SERVER_TAG=2.0.0-beta.16` pulls `durableworkflow/server:2.0.0-beta.16` from Docker Hub.
- `DW_SERVER_IMAGE=ghcr.io/durable-workflow/server:2.0.0-beta.16` pulls the same release
...
export DW_SERVER_TAG=2.0.0-beta.16
export DW_AUTH_TOKEN=dev-token
...
```env
DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.16
SERVER_PORT=8080
APP_ENV=production
APP_DEBUG=false
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-beta.16`, matching
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-beta.16` tag
...
`durable-workflow/workflow:2.0.0-beta.16` package and verifies that the tag
resolves to commit `68262eb8589e8e1142c2e158f50815950a347ef8` before the
...
git tag 2.0.0-beta.16 origin/v2
git push origin refs/tags/2.0.0-beta.16

# In the server repo, publish the Docker image tags.
git tag 2.0.0-beta.16 origin/main
git push origin refs/tags/2.0.0-beta.16
```
...
```bash
docker pull durableworkflow/server:2.0.0-beta.16
docker run --rm --entrypoint sh durableworkflow/server:2.0.0-beta.16 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'

docker pull ghcr.io/durable-workflow/server:2.0.0-beta.16
docker run --rm --entrypoint sh ghcr.io/durable-workflow/server:2.0.0-beta.16 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'
```
...
`durableworkflow/server:2.0.0-beta.16`. For production, patch every workload to the exact
