...
```bash
export DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.8
...
- `DW_SERVER_TAG=2.0.0-beta.8` pulls `durableworkflow/server:2.0.0-beta.8` from Docker Hub.
- `DW_SERVER_IMAGE=ghcr.io/durable-workflow/server:2.0.0-beta.8` pulls the same release
...
export DW_SERVER_TAG=2.0.0-beta.8
export DW_AUTH_TOKEN=dev-token
...
```env
DW_SERVER_IMAGE=durableworkflow/server:2.0.0-beta.8
SERVER_PORT=8080
APP_ENV=production
APP_DEBUG=false
...
The local compose files pass `WORKFLOW_PACKAGE_REF=2.0.0-beta.6`, matching
...
The Dockerfile clones the `durable-workflow/workflow` `2.0.0-beta.6` tag
...
`durable-workflow/workflow:2.0.0-beta.6` package and verifies that the tag
resolves to commit `5191a97d90e3e476c4e6a51e90faa559868e4c70` before the
...
git tag 2.0.0-beta.6 origin/v2
git push origin refs/tags/2.0.0-beta.6

# In the server repo, publish the Docker image tags.
git tag 2.0.0-beta.8 origin/main
git push origin refs/tags/2.0.0-beta.8
```
...
```bash
docker pull durableworkflow/server:2.0.0-beta.8
docker run --rm --entrypoint sh durableworkflow/server:2.0.0-beta.8 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'

docker pull ghcr.io/durable-workflow/server:2.0.0-beta.8
docker run --rm --entrypoint sh ghcr.io/durable-workflow/server:2.0.0-beta.8 -lc \
  'cat /app/.package-provenance && grep -n "serializer" /app/vendor/durable-workflow/workflow/src/config/workflows.php'
```
...
`durableworkflow/server:2.0.0-beta.8`. For production, patch every workload to the exact
