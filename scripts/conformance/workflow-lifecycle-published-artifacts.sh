...
write_php_sdk_runner_blocked() {
  local reason="${1:-PHP SDK lifecycle probe could not run.}"

  RESULT_DIR="$result_dir" \
  DW_WORKFLOW_PHP_VERSION="${DW_WORKFLOW_PHP_VERSION:-}" \
  PHP_SDK_RUNNER_BLOCKED_REASON="$reason" \
  node <<'NODE'
...
const reason = (process.env.PHP_SDK_RUNNER_BLOCKED_REASON || 'PHP SDK lifecycle probe could not run.').trim();
...
const payload = {
  ...
  scenario_results: {
    php_sdk_lifecycle_surface: {
      ...
      linked_findings: [
        {
          finding_id: 'workflow-lifecycle-php-sdk-lifecycle-surface-runner-gap',
          finding_type: 'conformance_runner_blocked',
          classification: 'runner-gap',
          scenario_id: 'php_sdk_lifecycle_surface',
          owning_surface: 'conformance_harness',
          summary: reason,
          next_acceptance_criterion: 'Run the PHP SDK lifecycle surface probe against the pinned Packagist artifact and record covered lifecycle cells plus typed refusals.',
        },
      ],
    },
  },
...
run_php_sdk_lifecycle_probe() {
  ...
  local sdk_version="${DW_WORKFLOW_PHP_VERSION:-}"
  ...
  local probe_dir="$result_dir/php-sdk-lifecycle-probe"
  mkdir -p "$probe_dir"

  cat > "$probe_dir/php-sdk-lifecycle-probe.php" <<'PHP'
<?php
...
use Workflow\V2\Exceptions\WorkflowTerminatedException;
use Workflow\V2\Exceptions\WorkflowTimeoutException;
use Workflow\V2\StartOptions;
use Workflow\V2\Support\ChildWorkflowOptions;
use Workflow\V2\Support\ChildWorkflowRetryPolicy;
...
function mark(string $cell): void
{
    global $coveredCells;
...
function unsupported_cell(string $cell, string $typedError, string $reason): void
{
    global $unsupportedCells, $typedErrors;

    $unsupportedCells[] = $cell;
    $typedErrors[] = [
        'cell' => $cell,
...
$installedVersion = InstalledVersions::getPrettyVersion('durable-workflow/workflow')
    ?: InstalledVersions::getVersion('durable-workflow/workflow')
    ?: '';
$artifactVersion = $installedVersion !== '' ? $installedVersion : $expectedVersion;

check(InstalledVersions::isInstalled('durable-workflow/workflow'), 'durable-workflow/workflow must be installed by Composer.');
check($artifactVersion !== '', 'installed durable-workflow/workflow artifact version must be discoverable.');
check(
    $expectedVersion === ''
        || ltrim($artifactVersion, 'v') === ltrim($expectedVersion, 'v'),
    sprintf('installed durable-workflow/workflow version %s did not match %s.', $artifactVersion, $expectedVersion),
);
mark('composer_packagist_artifact_imported');

foreach ([
    WorkflowClient::class,
    StartOptions::class,
    ContinueAsNewCall::class,
    ChildWorkflowOptions::class,
    ChildWorkflowRetryPolicy::class,
    WorkflowCancelledException::class,
    WorkflowTerminatedException::class,
    WorkflowTimeoutException::class,
] as $className) {
    check(class_exists($className), sprintf('%s must be exported by the Composer package autoloader.', $className));
}
mark('public_lifecycle_symbols_exported');

$http = new HttpFactory();
$http->fake(function (Request $request) use ($http, &$capturedRequests) {
    $capturedRequests[] = [
        'method' => strtoupper($request->method()),
        'url' => $request->url(),
        'body' => $request->data(),
        'namespace_header' => $request->hasHeader('X-Namespace', 'workflow-lifecycle-conformance'),
        'control_plane_header' => $request->hasHeader(WorkflowClient::CONTROL_PLANE_HEADER, WorkflowClient::CONTROL_PLANE_VERSION),
        'worker_protocol_header' => $request->hasHeader(WorkflowClient::WORKER_PROTOCOL_HEADER),
    ];

    if (str_contains($request->url(), '/signal/approve')) {
        return $http->response([
            'accepted' => true,
            'workflow_id' => 'wf-php-lifecycle',
            'command_status' => 'accepted',
        ]);
    }

    if (str_contains($request->url(), '/query/current')) {
        return $http->response([
            'success' => true,
            'result_envelope' => [
                'codec' => 'avro',
    ...
    return $http->response([
        ...
        'workflow_id' => 'wf-php-lifecycle',
        'run_id' => 'run-started',
        'workflow_type' => 'workflow.lifecycle.php',
    ]);
});
...
$started = $client->startWorkflow(
    'workflow.lifecycle.php',
    'wf-php-lifecycle',
    ['payload'],
    [
        'task_queue' => 'workflow-lifecycle-php',
        'duplicate_policy' => 'fail',
        'execution_timeout_seconds' => 300,
        'run_timeout_seconds' => 120,
    ],
);
$startRequest = $capturedRequests[array_key_last($capturedRequests)] ?? [];
$startBody = is_array($startRequest['body'] ?? null) ? $startRequest['body'] : [];

check(($started['workflow_id'] ?? null) === 'wf-php-lifecycle', 'WorkflowClient::startWorkflow must return the started workflow id.');
check(($startRequest['method'] ?? null) === 'POST', 'WorkflowClient::startWorkflow must use POST.');
check(($startRequest['url'] ?? null) === 'http://server:8080/api/workflows', 'WorkflowClient::startWorkflow must target the workflow start API.');
check(($startBody['workflow_id'] ?? null) === 'wf-php-lifecycle', 'WorkflowClient::startWorkflow must preserve workflow_id.');
check(($startBody['duplicate_policy'] ?? null) === 'fail', 'WorkflowClient::startWorkflow must forward duplicate_policy.');
check(($startBody['execution_timeout_seconds'] ?? null) === 300, 'WorkflowClient::startWorkflow must forward execution_timeout_seconds.');
check(($startBody['run_timeout_seconds'] ?? null) === 120, 'WorkflowClient::startWorkflow must forward run_timeout_seconds.');
check(($startBody['task_queue'] ?? null) === 'workflow-lifecycle-php', 'WorkflowClient::startWorkflow must forward task_queue.');
check(($startRequest['namespace_header'] ?? false) === true, 'WorkflowClient must include the selected namespace header.');
check(($startRequest['control_plane_header'] ?? false) === true, 'WorkflowClient must include the control-plane version header.');
check(($startRequest['worker_protocol_header'] ?? false) === true, 'WorkflowClient must include the worker protocol version header.');
check(
    isset($startBody['input']['blob'])
        && Serializer::unserializeWithCodec((string) ($startBody['input']['codec'] ?? 'avro'), (string) $startBody['input']['blob']) === ['payload'],
    'WorkflowClient::startWorkflow must encode input arguments as a payload envelope.',
);
mark('workflow_client_start_with_duplicate_policy_and_timeout_budgets');

$signal = $client->signalWorkflow('wf-php-lifecycle', 'approve', ['ok']);
check(($signal['command_status'] ?? null) === 'accepted', 'WorkflowClient::signalWorkflow must expose accepted command status.');
mark('workflow_client_signal_command_surface');

$query = $client->queryWorkflow('wf-php-lifecycle', 'current');
check($query === ['state' => 'ready'], 'WorkflowClient::queryWorkflow must decode result envelopes.');
mark('workflow_client_query_result_envelope');

$startOptions = StartOptions::rejectDuplicate()
    ->withExecutionTimeout(300)
    ->withRunTimeout(120);
check($startOptions->duplicateStartPolicy === DuplicateStartPolicy::RejectDuplicate, 'StartOptions::rejectDuplicate must preserve reject semantics.');
check($startOptions->executionTimeoutSeconds === 300, 'StartOptions must preserve execution timeout budgets.');
check($startOptions->runTimeoutSeconds === 120, 'StartOptions must preserve run timeout budgets.');
mark('start_options_duplicate_policy_and_timeout_budgets');

try {
    new StartOptions(executionTimeoutSeconds: 0);
    check(false, 'StartOptions must reject zero execution timeout.');
} catch (LogicException $exception) {
    $typedErrors[] = [
        'cell' => 'invalid_workflow_execution_timeout',
        'typed_error' => LogicException::class,
        'refusal_reason' => $exception->getMessage(),
        'documented' => true,
    ];
    mark('start_options_timeout_typed_refusal');
}

$continueAsNew = new ContinueAsNewCall(['next', 2]);
check($continueAsNew->arguments === ['next', 2], 'ContinueAsNewCall must preserve continue-as-new arguments.');
mark('continue_as_new_command_surface');

$childOptions = new ChildWorkflowOptions(ParentClosePolicy::Terminate, queue: 'workflow-lifecycle-child');
$childSnapshot = $childOptions->toSnapshot();
$childRetry = ChildWorkflowRetryPolicy::snapshotExternal([
    'max_attempts' => 3,
    'backoff_seconds' => [1, 2],
    'non_retryable_error_types' => ['DomainException'],
]);
$childTimeouts = ChildWorkflowRetryPolicy::timeoutSnapshot(300, 120);
check(($childSnapshot['parent_close_policy'] ?? null) === 'terminate', 'ChildWorkflowOptions must preserve terminate parent-close policy.');
check(($childSnapshot['queue'] ?? null) === 'workflow-lifecycle-child', 'ChildWorkflowOptions must preserve child task queue routing.');
check(($childRetry['max_attempts'] ?? null) === 3, 'ChildWorkflowRetryPolicy must preserve max attempts.');
check(($childRetry['backoff_seconds'] ?? null) === [1, 2], 'ChildWorkflowRetryPolicy must preserve backoff seconds.');
check(($childTimeouts['execution_timeout_seconds'] ?? null) === 300, 'Child workflow timeout snapshot must preserve execution timeout.');
check(($childTimeouts['run_timeout_seconds'] ?? null) === 120, 'Child workflow timeout snapshot must preserve run timeout.');
mark('child_workflow_retry_timeout_and_parent_close_policy');

check(method_exists(Workflow\V2\WorkflowStub::class, 'cancel'), 'WorkflowStub must expose cancel().');
check(method_exists(Workflow\V2\WorkflowStub::class, 'terminate'), 'WorkflowStub must expose terminate().');
mark('workflow_stub_cancel_terminate_methods');

$cancelled = new WorkflowCancelledException('operator requested cancellation');
$terminated = new WorkflowTerminatedException('operator requested termination');
$timeout = WorkflowTimeoutException::runTimeout('2026-07-01T00:00:30Z');
check($cancelled instanceof Error && ! $cancelled instanceof Exception, 'WorkflowCancelledException must be a typed non-Exception throwable.');
check($terminated instanceof RuntimeException, 'WorkflowTerminatedException must be a typed RuntimeException.');
check($timeout->timeoutKind === 'run_timeout', 'WorkflowTimeoutException must expose timeout kind.');
mark('workflow_terminal_exception_contract');

$retryHttp = new HttpFactory();
$retryHttp->fake(fn (Request $request) => $retryHttp->response([
...
$retryClient = new WorkflowClient($retryHttp, 'http://server:8080/', 'test-token', 'workflow-lifecycle-conformance');

try {
    $retryClient->startWorkflow(
        'workflow.lifecycle.php',
        'wf-php-retry',
        [],
        [
    ...
    check($exception->statusCode() === 422, 'WorkflowClientException must expose HTTP 422 for unsupported retry_policy.');
    unsupported_cell('workflow_level_retry_policy', WorkflowClientException::class, $exception->getMessage());
}

$coveredCells = array_values(array_unique($coveredCells));
sort($coveredCells);
$unsupportedCells = array_values(array_unique($unsupportedCells));
sort($unsupportedCells);
...
$payload = [
    ...
    'scenario_results' => [
        'php_sdk_lifecycle_surface' => [
            'scenario_id' => 'php_sdk_lifecycle_surface',
            'status' => $status,
            'classification' => $status === 'pass' ? 'passed' : 'product-gap',
            'published_artifact_cell_executed' => true,
            'observed_outputs' => [
                'sdk' => 'workflow-php',
                ...
                'artifact_source' => 'packagist://durable-workflow/workflow@'.$artifactVersion,
                'composer_package' => 'durable-workflow/workflow',
                ...
                'captured_request_paths' => array_map(
                    static fn (array $request): string => parse_url((string) ($request['url'] ?? ''), PHP_URL_PATH) ?: '',
                    $capturedRequests,
                ),
                'published_artifact_cell_executed' => true,
                'local_product_source_checkouts_used' => false,
            ...
            'linked_findings' => $status === 'pass' ? [] : [$finding],
        ],
    ],
];
