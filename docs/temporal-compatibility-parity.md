...
| # | Capability | Status | Priority | Evidence / notes |
|---|---|---|---|---|
| 9 | **Rust SDK** — first-party Rust support for Workflows + Activities | public preview | P2 | A publishable Rust crate now lives under `sdk-rust/` with client start/signal APIs, worker registration using `runtime=rust`, workflow/activity task polling and completion, worker/activity heartbeats, and the same JSON-native `avro` generic payload wrapper used by the PHP and Python SDKs. The hello-world example in `sdk-rust/examples/hello_world.rs` exercises a start + signal + workflow + activity round-trip against the standalone server. |
