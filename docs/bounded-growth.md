...
| `long_poll_wait_slots` | `server:long-poll-wait-slot:` | `App\Support\LongPollWaitSlotStore` | One held-wait slot per admitted empty worker long-poll, capped per server node so health and control-plane request workers stay available. |
