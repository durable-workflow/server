...
| `dw_namespace_durable_state_usage` | `GET /api/system/metrics` | `namespace` is request-scoped rather than a label. Durable usage uses six fixed resource fields; rejection counters use six exhaustion reasons plus quota unavailability. |
