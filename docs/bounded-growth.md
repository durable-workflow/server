...
| `dw_workflow_runs_total` | `GET /api/system/prometheus-metrics` | `namespace` is request-scoped rather than a label. `task_queue`/`workflow_type` series are limited by `server.metrics.prometheus_workflow_series_limit`, default 100 and hard-clamped to 500; `cardinality.series_limits.workflows` reports observed, reported, truncated, suppressed series, and suppressed started totals. |
...
| `dw_task_queue_runtime_state` | `GET /api/system/prometheus-metrics` | `namespace` is request-scoped rather than a label. `task_queue` series are limited by `server.metrics.prometheus_task_queue_series_limit`, default 100 and hard-clamped to 500; `cardinality.series_limits.task_queues` reports observed, reported, truncated, and suppressed queue series. |
