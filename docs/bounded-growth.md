...
| `long_poll_wait_slots` | `server:long-poll-wait-slot:` | `App\Support\LongPollWaitSlotStore` | One held-wait slot per admitted empty workflow/activity worker long-poll, capped per server node so health and control-plane request workers stay available. Query-task polls are excluded so live workflow queries are not starved by idle workflow/activity waits. |
