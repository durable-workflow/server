...
| `timer_runtime_contract` (server side handoff) | `GET /api/cluster/info`'s `timer_runtime_contract` manifest and the public scenario manifest at `static/platform-conformance/timer-runtime-scenarios.json`, which define the non-passing runner-gap handoff for concurrent timer ordering, cancellation while waiting, and operator-visible timer waiting state evidence | runner-gap |
